...
class Demo:
    ...
    def test(self, type):
        tests = [self.random_test, self.rating_test, self.compound_test]

        self.engine = RunEngine(tests[1](), initial_state=[1,2,3,4,5,5,6,4,7,8,3,345,5])

        self.engine.start_engine()
        self.engine.run_engine()
        print()
        print(self.engine.get_seed())
        print(self.engine.get_seed(True))
        print("\n\n")
        self.engine.rebuild()
        self.engine.run_engine()
